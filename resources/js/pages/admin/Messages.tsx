...
export default function AdminMessages() {
    ...
    const filterStatus = (status: string) => {
        router.get('/admin/messages', status ? { status } : {}, { preserveState: true, replace: true });
    };

    return (
        <AppLayout breadcrumbs={[{ title: 'Admin', href: '/admin' }, { title: 'Messages', href: '/admin/messages' }]}>
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-1">
                                            ...
                                            {msg.phone && <span className="text-sm text-muted-foreground">{msg.phone}</span>}
                                            <span className={cn('rounded-full px-2.5 py-0.5 text-xs font-semibold', STATUS_COLORS[msg.status] ?? 'bg-muted text-muted-foreground')}>
                                                {msg.status}
                                            </span>
                                        </div>
                                        {msg.subject && (
                                            <div className="mb-2 text-sm font-medium text-foreground">{msg.subject}</div>
                                        )}
                                    ...
                                    <div className="flex flex-col items-end gap-2 shrink-0">
                                        ...
                                        <select
                                            value={msg.status}
                                            onChange={(e) => updateStatus(msg, e.target.value)}
