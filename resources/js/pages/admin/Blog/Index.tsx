...
export default function AdminBlogIndex() {
    ...
    const filterStatus = (status: string) => {
        router.get('/admin/blog', status ? { status } : {}, { preserveState: true, replace: true });
    };

    return (
        <AppLayout breadcrumbs={[{ title: 'Admin', href: '/admin' }, { title: 'Blog', href: '/admin/blog' }]}>
                    ...
                    <table className="w-full text-sm">
                        <thead className="border-b border-border bg-muted/50">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left font-semibold text-foreground">Auteur</th>
                                <th className="px-4 py-3 text-left font-semibold text-foreground">Publication</th>
                                ...
                                posts.data.map((post) => (
                                    <tr key={post.id} className="hover:bg-muted/30">
                                        <td className="px-4 py-3">
                                            ...
                                            <div className="text-xs text-muted-foreground">/blog/post/{post.slug}</div>
                                        </td>
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={cn('rounded-full px-2.5 py-1 text-xs font-semibold', STATUS_COLORS[post.status] ?? 'bg-muted text-muted-foreground')}>
